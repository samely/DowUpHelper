package org.openstreetmap.pluging.Download;
...
public class DownloadOsmTask extends AbstractDownloadTask {
     ...
     *
     * Set <code>progressMonitor</code> to null, if the task should create,
     * open, and close a progress monitor. Set progressMonitor to
     * {@link NullProgressMonitor#INSTANCE} if progress information is to be
     * discarded.
     *
     * You can wait for the asynchronous download task to finish by
     * synchronizing on the returned {@link Future}, but make sure not to freeze
     * up JOSM. Example:
     * <pre>
     ...
     * </pre>
     *
     * The following example uses a pattern which is better suited if a task is
     * launched from the Swing EDT:
     * <pre>
     ...
     * </pre>
     *
     * @param reader the reader used to parse OSM data (see
     * {@link OsmServerReader#parseOsm})
     * @param newLayer true, if the data is to be downloaded into a new layer.
     * If false, the task selects one of the existing layers as download layer,
     * preferably the active layer.
