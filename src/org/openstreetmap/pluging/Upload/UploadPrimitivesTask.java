package org.openstreetmap.pluging.Upload;
...
public class UploadPrimitivesTask extends AbstractUploadTask {
     ...
     * @param layer the OSM data layer for which data is uploaded. Must not be
     * null.
     * @param toUpload the collection of primitives to upload. Set to the empty
     * collection if null.
     * @param changeset the changeset to use for uploading. Must not be null.
     * changeset.getId() can be 0 in which case the upload task creates a new
     * changeset
    ...
    public UploadPrimitivesTask(UploadStrategySpecification strategy, OsmDataLayer layer, APIDataSet toUpload, Changeset changeset) {
        super(tr("Uploading data for layer ''{0}''", layer.getName()), NullProgressMonitor.INSTANCE, false);
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    pm.subTask(trn("Uploading {0} object...", "Uploading {0} objects...", toUpload.getSize(), toUpload.getSize()));

                    synchronized (this) {
                        writer = new OsmServerWriter();
                    }

                    writer.uploadOsm(strategy, toUpload.getPrimitives(), changeset, null);
                    ...
                    //
                    recoverFromGoneOnServer(e, null);
            ...
            if (strategy.isCloseChangesetAfterUpload() && changeset != null && !changeset.isNew() && changeset.isOpen()) {
                OsmApi.getOsmApi().closeChangeset(changeset, null);
            }
        } catch (Exception e) {
            if (uploadCanceled) {
                    ...
                    if (e.getSource().equals(ChangesetClosedException.Source.UPLOAD_DATA)) {
                        switch (strategy.getPolicy()) {
                            ...
                            case FILL_ONE_CHANGESET_AND_RETURN_TO_UPLOAD_DIALOG:
                                ...
                                UploadDialog.getUploadDialog().setVisible(false);
                                break;
                        }
                    } else {
                        handleFailedUpload(lastException);
